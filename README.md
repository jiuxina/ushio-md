...
- 智能过滤：自动清理不存在的文件引用
- 输入验证：重命名时自动检测非法字符
- 外部文件导入：可选"仅查看"（直接在原路径编辑）或"导入"（连同引用图片一起复制到工作区）
- 此外，请等待后续更新……
...
- 内置官方插件市场，一键安装/更新
- 支持 GitHub 风格 Alert 渲染（NOTE/TIP/IMPORTANT/WARNING/CAUTION，共 5 种）
- 开发者友好，提供完整开发文档
...
| 问题 | 说明 |
| ---- | ---- |
| "仅查看"外部文件保存限制 | 以"仅查看"方式打开工作区外的文件时，能否回写源文件取决于 Android 存储权限；如需可靠保存，建议使用"导入"功能将文件复制到工作区 |
