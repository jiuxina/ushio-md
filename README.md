...
- 智能过滤：自动清理不存在的文件引用
- 输入验证：重命名时自动检测非法字符
- 外部文件导入：支持"直接打开"（在原路径编辑）或"导入副本"（复制到工作区）
- 此外，请等待后续更新……
...
| 问题 | 说明 |
| ---- | ---- |
| 云同步不稳定 | 云同步功能目前无法保证正常使用，待后续版本优化 |
| 图片路径重复 | 导入图片后，文件会同时复制至 picture 目录和文档目录，实际引用的是文档目录下的文件 |
| 导出长图样式旧 | 导出长图时使用的是旧版样式，待后续更新 |
| 直接打开无法回写 | 通过"直接打开"方式打开外部文件时，暂无法将修改保存回源文件，建议使用"导入副本"功能 |
