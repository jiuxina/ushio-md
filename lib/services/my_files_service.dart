...
class MyFilesService {
  ...
  Future<String> getWorkspacePath() async {
    ...
    // 否则使用平台默认的文档目录
    final documentsDir = await _getDefaultDocumentsDirectory();
    if (documentsDir == null) {
      throw Exception('无法获取文档目录');
    }

    _workspacePath =
        '${documentsDir.path}${Platform.pathSeparator}$workspaceName';
    return _workspacePath!;
  }
  ...
  Future<String> copyDocumentWithImages(String sourcePath) async {
    ...
    for (final match in matches) {
      ...
      if (imagePath.startsWith('/') || imagePath.contains(':')) {
        ...
        // 相对路径，转为绝对路径
        absolutePath =
            '$sourceDir${Platform.pathSeparator}${imagePath.replaceAll('/', Platform.pathSeparator)}';
      }
      ...
      content = content.replaceAll(
        RegExp(
          r'!\[([^\]]*)\]\(' +
              RegExp.escape(
                originalPath
                    .replaceAll(sourceDir + Platform.pathSeparator, '')
  ...
  Future<String> copyImageToDocument(
    String imagePath,
    String documentPath,
  ) async {
  ...
  /// 列出工作区中的所有文件和文件夹
  Future<List<FileSystemEntity>> listWorkspaceContents({
    String? subPath,
  }) async {
    final workspacePath = await getWorkspacePath();
