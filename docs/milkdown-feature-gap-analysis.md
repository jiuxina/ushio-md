...
3. **内容回写策略优化**
   ...
   - ✅ 已完成第三步：引入最小防抖窗口（120ms）进行全量 markdown 回写节流，降低高频编辑桥接压力。
   - 后续可选：在 Flutter 消费模型可兼容前提下评估增量回写。

4. **插件能力继续扩展**
   - ✅ 已完成第四步首批扩展：`tooltip` 增加“插入表格/插入图片”动作；`slash` 增加“表格 5x5/图片”动作并保留原动作。
   - 后续可继续对齐更多工具栏动作（如链接、分割线、任务列表等）与参数化配置。
