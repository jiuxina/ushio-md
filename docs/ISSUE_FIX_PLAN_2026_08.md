...
## 4.2 模拟器人工验证记录（2026-08-10）

验证环境：`emulator-5556`，Android 12（API 32），调试 APK 包名 `com.ushiomd`；通过 `appops` 授予
`MANAGE_EXTERNAL_STORAGE`，SAF 目录授权到 `/storage/emulated/0/md_test`。

- 重命名：新建 `rename_noext` 后通过文件菜单重命名为 `renamed_ui`（不输入 `.md`），列表显示
  `renamed_ui.md`，`adb` 落盘确认文件名正确且未消失。
- 特殊符号往返：`symbol_test.md` 含 `–`、`—`、中文引号、省略号、连续空行、引用块与列表；预览 → 编辑 →
  预览往返后文件内容与原文一致；在编辑模式做一次真实修改并保存后，特殊字符均保留，未出现符号篡改。
- 引用块间距：视觉理解桥确认引用块左边框约 4-5px，文字距边框约 16-24px，边框上下各超出文字约 4-6px，
  符合“增加呼吸空间”的预期。
- 键盘遮挡：80 段长文档打开并弹出输入法（`mInputShown=true`）后，内容可正常滚动（可见段落从 1-5 变为
  4-9），最后可见行位于键盘上方、未被遮挡。
- 性能：15,597 字符文档正常打开，编辑模式执行标题格式命令后界面正常响应、字符统计更新，未观察到明显
  卡顿（定性确认；精确帧率基准未做）。
- 备注：自动化脚本曾触发一次 Flutter 红屏（`InheritedElement._dependents.isEmpty`），复现路径为对对话框
  外误点后再点击确定；正常人工操作路径未复现，作为低概率边界观察保留。
...
- [x] 长文 + 中文输入法弹出时当前行可见、可滚动（模拟器确认）
- [x] 含 `–`、中文引号、连续空行的文档预览/编辑往返内容一致（模拟器确认）
- [x] 新建文件重命名不带 `.md` 时文件仍显示，且自动带 `.md`（模拟器确认）
- [x] 引用块左右/上下间距符合预期（模拟器确认）
- [x] 长文档（15,597 字符）打开/编辑无卡顿（定性确认）
