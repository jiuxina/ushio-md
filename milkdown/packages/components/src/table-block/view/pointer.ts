...
function createPointerMoveHandler(
  ...
  return throttle((e: PointerEvent) => {
    ...
    // Increased threshold from 8px to 16px for better touch screen experience
    const threshold = 16
    const closeToBoundaryLeft = Math.abs(e.clientX - boundary.left) < threshold
    const closeToBoundaryRight = Math.abs(boundary.right - e.clientX) < threshold
    const closeToBoundaryTop = Math.abs(e.clientY - boundary.top) < threshold
    const closeToBoundaryBottom = Math.abs(boundary.bottom - e.clientY) < threshold
